...
import { getRotateMatrix, getScaleMatrix, getTranslateMatrix, mixMatrix } from '@/utils/webgl/matrix.js'
...
onMounted(() => {
  ...
  const animation = () => {
    ...
    const mix = mixMatrix(mixMatrix(translate, scale), rotate)

    // uniformMatrix4fv 用于设置 uniform mat 变量值为矩阵值
    // uniformMatrix4fv 参数 2 在 webgl 中恒为 false
    gl.uniformMatrix4fv(mat, false, mix)
