...
onMounted(() => {
  ...
  canvas.addEventListener('mousemove', (e) => {
    ...
    // 获取坐标轴比例
    const proportionX = (x - halfWidth) / halfWidth
    const proportionY = (halfHeight - y) / halfHeight

    pointArr.push({ x: proportionX, y: proportionY })

    // 绘制一个点
    // gl.vertexAttrib2f(aPosition, proportionX, proportionY)
    // gl.drawArrays(gl.POINTS, 0, 1)
